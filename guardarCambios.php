...
$imge = addslashes(file_get_contents($img));*/
	...
	$conectar=Conectarse();
	$agregarS='UPDATE servicios SET id_servicio="'.$id_servicio.'", nombre_dpto="'.$dpto.'", desc_servicio="'.ucwords($descservicio).'",
	id_user="'.$id_user.'", fecha_solicitud="'.$fechaInicio.'", fecha_realizacion="'.$fechaFinal.'", nombre_responsable="'.ucwords($respservicio).'", nombre_solicitante="'.ucwords($nombresol).'" where id_servicio="'.$id_serv.'"';
